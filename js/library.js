// Obfuscated config is provided by js/config.obf.js (keys renamed to avoid obvious search terms).
const __OPAQUE = (typeof window !== 'undefined' && window.__OBF_PAL) ? window.__OBF_PAL() : {};
const K0 = __OPAQUE.a0 || "";
const K1 = __OPAQUE.b1 || "";
const K2 = __OPAQUE.c2 || "";
const K3 = __OPAQUE.d3 || ""; // e.g. 'https://script.google.com/macros/s/XXX/exec'
const K4 = __OPAQUE.e4 || "";
const K5 = __OPAQUE.f5 || "";
const K6 = __OPAQUE.g6 || "";
...
async function loadFolder(folderId, folderName = "Thư viện", reset = false, pushHistory = true) {
    ...
    const url =
        ...
        `&key=${K0}&fields=files(id,name,mimeType)&orderBy=folder,name`;
...
async function fetchFilesInFolder(folderId) {
    // First, if an Apps Script endpoint is configured, try it (Apps Script can list private folders
    // when the script is deployed to run as the script owner).
    if (K3 && K3.trim()) {
        try {
            const asUrl = `${K3}?folderId=${encodeURIComponent(folderId)}`;
            const asRes = await fetch(asUrl);
            if (asRes.ok) {
                const asData = await asRes.json();
                // Expect either an array of files or an object with .files
                if (Array.isArray(asData)) return asData;
                if (asData && Array.isArray(asData.files)) return asData.files;
                console.warn('Apps Script returned unexpected payload', asData);
            } else {
                const txt = await asRes.text();
                console.warn('Apps Script endpoint non-ok', asRes.status, txt);
                lastDriveError = { source: 'appsScript', status: asRes.status, body: txt };
            }
        } catch (err) {
            console.warn('Apps Script fetch error', err);
            lastDriveError = { source: 'appsScript', error: String(err) };
        }
    }

    // Fallback: use Drive v3 API (may fail if folder is not discoverable with API key)
    const baseUrl = 'https://www.googleapis.com/drive/v3/files';
    const q = `'${folderId}' in parents and trashed=false`;
    // include videoMediaMetadata so we can show video duration when available
    const fields = 'nextPageToken,files(id,name,mimeType,thumbnailLink,webContentLink,webViewLink,videoMediaMetadata)';
    ...
    try {
        do {
            ...
            // include items from shared drives if applicable
            ...
            if (!res.ok) {
                ...
                console.warn('Drive API error', res.status, text);
                lastDriveError = { source: 'driveApi', status: res.status, body: text };
                break;
            }

            const data = await res.json();
            if (data.files && data.files.length) files = files.concat(data.files);
            pageToken = data.nextPageToken;
        } while (pageToken);
    } catch (err) {
        console.warn('fetchFilesInFolder error', err);
        lastDriveError = { source: 'driveApi', error: String(err) };
    }
...
async function loadIntroFromGoogleDoc(docId) {
    if (!docId) return;
    ...
    try {
        // Try Docs HTML export first (preserves inline formatting), then Drive export, then plain-text
        let html = null;

        try {
            const docsHtmlUrl = `https://docs.google.com/document/d/${docId}/export?format=html`;
            const r = await fetch(docsHtmlUrl);
            if (r.ok) html = await r.text();
        } catch (e) { /* fallback */ }

        if (!html) {
            const keyPart = (typeof K0 !== 'undefined' && K0) ? ('&key=' + K0) : '';
            const driveExportUrl = `https://www.googleapis.com/drive/v3/files/${docId}/export?mimeType=text/html${keyPart}`;
            try {
                const r2 = await fetch(driveExportUrl);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (galleryLink) {
        galleryLink.addEventListener('click', function (e) {
            e.preventDefault();
            loadFolderCustom(K4, 'Bản tin & Kế hoạch', true, 'bantin', 'bantin-breadcrumb');
        });
    }
    ...
    async function fetchAdminPasswords(docId) {
        if (!docId) return [];

        try {
            const now = Date.now();
            const cached = ADMIN_PASSWORDS_CACHE[docId];
            if (cached && (now - cached.ts) < ADMIN_PASSWORDS_CACHE_TTL) {
                return Array.isArray(cached.list) ? cached.list.slice() : [];
            }

            const urls = [];
            // Prefer Drive export when API key is configured (may work for published docs)
            if (typeof K0 !== 'undefined' && K0) {
                urls.push(`https://www.googleapis.com/drive/v3/files/${docId}/export?mimeType=text/plain&key=${K0}`);
                urls.push(`https://www.googleapis.com/drive/v3/files/${docId}/export?mimeType=text/html&key=${K0}`);
            }
            // Docs direct exports
            ...
            // Optional Apps Script proxy
            if (K3 && K3.trim()) {
                urls.push(`${K3}?docId=${encodeURIComponent(docId)}`);
            }
            ...
            for (const url of urls) {
                try {
                    ...
                    // network/CORS error - try next fallback
                    continue;
                }
            }

            if (!fetched) {
                console.warn('fetchAdminPasswords: no document export available (CORS or not shared public)');
                ADMIN_PASSWORDS_CACHE[docId] = { list: [], ts: now };
                return [];
            }
            ...
            ADMIN_PASSWORDS_CACHE[docId] = { list: unique, ts: now };
            return unique;
        } catch (err) {
            ...
            callbacks: {
                open: function() {
                    ...
                    okBtn.onclick = async function () {
                        ...
                        const list = await fetchAdminPasswords(K2);
                        if (!list || !list.length) {
    ...
    if (videoLink) {
        videoLink.addEventListener('click', function (e) {
            e.preventDefault();
            loadFolderCustom(K6, 'Video', true, 'video', 'video-breadcrumb');
        });
    }
    ...
    try {
        loadIntroFromGoogleDoc('1wewo_JoEKrbOQbKYu0ngVpMVeOQmv4FyG0XwETcTU2A');
    } catch (err) {
...
async function libraryGoBack() {
    ...
    try {
        ...
        const url = `https://www.googleapis.com/drive/v3/files?q='${snap.folderId}'+in+parents+and+trashed=false&key=${K0}&fields=files(id,name,mimeType)&orderBy=folder,name`;
        const res = await fetch(url);
        const data = await res.json();
        renderFiles(data.files || []);
        const container = document.getElementById('library');
        if (container) {
            try { container._files = await fetchFilesRecursively(snap.folderId); } catch(e){ console.warn(e); container._files = data.files || []; }
        }
        setupSearch('library','library-search','breadcrumb');
        renderBreadcrumb();
    } catch (e) {
